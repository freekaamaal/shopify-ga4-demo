(function () {
  try {
    ...
    // --- figure out our base so we can load siblings (rules + stage2 files)
    ...
    // --- read query on the script src (supports ?debug=1&v=123)
    var SQS = new URLSearchParams((SRC.split('?')[1]) || '');
    var DEBUG   = SQS.get('debug') === '1';
    var VERSION = SQS.get('v') || '';

    // --- rules.json (cache-busted with v=)
    var RULES_URL = BASE + '/rules.json' + (VERSION ? ('?v=' + encodeURIComponent(VERSION)) : '');
    var TIMEOUT_MS = 1000; // give GH Pages/CDNs time
    ...
    // - accepts raw "^/products?/.*"
    // - accepts "/^\\/products\\//i"
    function compileRe(s){
      if (typeof s !== 'string' || !s) return null;
      try{
        if (s[0] === '/' && s.lastIndexOf('/') > 0){
          var last = s.lastIndexOf('/'); var body = s.slice(1,last), flags = s.slice(last+1);
          return new RegExp(body, flags);
        }
        return new RegExp(s);
      }catch(_){ return null; } // fallback: no regex
    }
    function reListHit(list, str){
      if (!list || !list.length) return true; // no constraint -> pass
      for (var i=0;i<list.length;i++){
        var raw = list[i], rx = compileRe(raw);
        try{
          if (rx ? rx.test(str) : (String(str).indexOf(String(raw).replace(/\//g,'')) !== -1)) return true;
        }catch(_){}
      }
      return false;
    }
    ...
    // --- load rules.json (soft fail -> keep)
    withTimeout(fetch(RULES_URL, { credentials:'omit', cache:'no-store' })
      .then(function(r){ return r && r.ok ? r.json() : null; }), TIMEOUT_MS)
    .then(function(cfg){
      var action = 'keep';
      var utm = null;
      var preserveClickIds = true;
      var matched = null;

      if (cfg && !cfg.kill_switch){
        preserveClickIds = (cfg.defaults && typeof cfg.defaults.preserveClickIds === 'boolean')
                           ? cfg.defaults.preserveClickIds : true;
        var rules = cfg.rules || [];
    ...
    try { console.warn('stage1 error', e); } catch (_) {}
  }
})();
